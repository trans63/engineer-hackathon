...
setInterval(function () {
  var poll = polls[Math.floor(Math.random() * polls.length)];

  if (!poll.percentComplete) {
    poll.status = 'Running';
  }

  _notifiers.poll.forEach(function (notifier) {
  ...
  polls.forEach(function (poll) {
    if (poll.status === 'Running') {
      if (poll.percentComplete < 100) {
        poll.percentComplete = Math.min(100, poll.percentComplete + increments[Math.floor(Math.random() * increments.length)]);
      } else {
...
function getPolls(filters) {
  return axios.get('http://localhost:3001/state').then(function (response) {
...
exports.default = { addNotifier: addNotifier, addSession: addSession, getSession: getSession, getPoll: getPoll, getPolls: getPolls };
